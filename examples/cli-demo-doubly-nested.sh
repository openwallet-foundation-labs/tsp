...
sleep 0.2 && tsp --wallet marlon request -s marlon -r marc &
...
sleep 2 && read -d '' nested_marlon nested_marc <<< $(tsp --wallet marlon request --nested -s marlon -r marc)
...
sleep 2 && read -d '' nested_nested_marlon nested_nested_marc <<< $(tsp --wallet marlon request --nested -s "$nested_marlon" -r "$nested_marc" -p "$marlon")
