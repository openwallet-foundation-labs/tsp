...
//! `cargo run -p tsp_sdk --example generate_test_vectors`. They are not
//! reproducible by re-running that: the sealed box draws an ephemeral key pair
//! and HPKE draws encapsulation randomness. A vector is verified by opening it
//! with the private keys the file carries, not by re-encrypting and comparing.
...
mod test {
    ...
    fn every_vector_opens_to_what_it_says_it_does() {
        ...
        assert_eq!(vectors.vectors.len(), 8, "every vector was exercised");
    }
